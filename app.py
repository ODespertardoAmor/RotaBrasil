...
def nova_corrida():
    try:
        ...
        nova = Corrida(
            ...
            valor=valor_corrida,
            distancia=str(dados.get("distancia", "")),
        ...
        # Dados para enviar aos motoristas
        dados_chamada = {
            "corrida_id": nova.id,
            ...
            "valor": nova.valor,
            "distancia": str(dados.get("distancia", "Calculando...")),
            "forma_pagamento": forma_pagamento,
            "lat_origem": lat_origem,
            "lon_origem": lon_origem,
            "lat_destino": lat_destino,
...
def receber_localizacao(dados):
    ...
    if motorista_id and lat and lng:
        try:
            # 🔥 SALVA NO BANCO DE DADOS
            db.session.execute(
                db.text("""
                ...
                """),
                {
                    'cid': int(corrida_id) if corrida_id else 0, 
            ...
            db.session.commit()
            print(f"💾 Localização SALVA no banco - Motorista {motorista_id}: {lat}, {lng}")
            
        except Exception as e:
            db.session.rollback()
            print(f"❌ Erro ao salvar localização: {e}")
        
        # Emite para o passageiro (se tiver corrida)
        if corrida_id and int(corrida_id) > 0:
            ...
            print(f"📍 Emitido para sala corrida_{corrida_id}")
...
def admin_atualizar_foto():
    try:
        ...
        if not usuario_id or not foto_perfil:
